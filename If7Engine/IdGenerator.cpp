...
QString IdGenerator::face(const QString & className) const
{
    QString id = generate(className, className);
    if (id.isEmpty())   id = generate("Face", className);
    return id;
}

QString IdGenerator::frame(const QString & className) const
{
    QString id = generate(className, className);
...
QString IdGenerator::generate(const QString & className,
                              const QString & outputClass) const
{
    //pathSub = '/';
    QString result;
    QString format = sortable_format_map[className.toLower()];
    if (format.isEmpty()) return QString();
