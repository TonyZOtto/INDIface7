...
//        MsecGenerate = localTime.elapsed();
    ...
    if (differences.contains(EigenFaceVectorKey()))
    {
        ...
        fInfo.templateMethod(methodString());
        fInfo.consistency(vec.consistency());
...
Return EigenFaceGenerator::findHead(void)
{
    ...
    else
    {
        ...
        fInfo.set(Feature::HeadMethod, ffd->methodString());
        fInfo.set(Feature::HeadBox,
...
Return EigenFaceGenerator::findEyes(void)
{
    ...
    fInfo.set(Feature::EyeMethod, leyed->methodString()
                                      + "|" + leyed->methodString());
...
//    TRACE("originalImage=%1,%2 normalSize=%3,%4", originalImage.width(), originalImage.height(), normalSize.width(), normalSize.height());
  //  TRACE("adjEyes=%1,%2~%3,%4", adjEyes.left().x(), adjEyes.left().y(), adjEyes.right().x(), adjEyes.right().y());
    ...
    fInfo.eyeLine((QLine)normalEyes);
