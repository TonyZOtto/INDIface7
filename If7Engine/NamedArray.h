...
template <class NAMED, class T> class  NamedArray : public QList<T>
{
public:
    NamedArray(void)
    {
    ...
    unsigned count() const { return NAMED::count(); }
    T & at(const int Index) const { return NAMED::at(Index); }

    bool isValidIndex(const int Index) const
    {
        return ((Index >= 0) && (Index < NAMED::count()));
    }
